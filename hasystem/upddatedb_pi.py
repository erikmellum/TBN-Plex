...
def getmovies():
	...
	for video in mxlist:
		...
		try:
			...
			if not cur.fetchone():
				cur.execute('INSERT INTO Movies VALUES(?, ?, ?, ?, ?, ?, ?)', (sqlite3.Binary(name), summary, rating, tagline, bgenre, directors, bactors))
				sql.commit()
		except IndexError:
			print ("\nError adding " + name)
...
def getshows():
	...
	for show in tvlist:
		name = str(show.title.encode('utf-8'))
		...
		if not cur.fetchone():
			cur.execute("INSERT INTO TVshowlist VALUES (?,?,?,?,?,?)",(sqlite3.Binary(name),summary,bgenre,rating,duration,totalnum))
			sql.commit()
